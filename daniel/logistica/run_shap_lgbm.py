...
def main() -> int:
    p = argparse.ArgumentParser(description="Compute SHAP explanations for a LightGBM bundle.")
    p.add_argument("--input", required=True, help="Path to input data (CSV or Parquet).")
    p.add_argument("--bundle", default="model_bundle.joblib", help="Path to model bundle joblib.")
    p.add_argument("--outdir", default=os.path.join("shap", "out"), help="Output directory.")
    p.add_argument("--topk", type=int, default=3, help="Top-k positive/negative drivers to export.")
    p.add_argument("--background", type=int, default=200, help="Rows to use as SHAP background dataset (sampled).")
    args = p.parse_args()
    ...
    print(f"OK. Arquivos gerados em: {args.outdir}")
    return 0
